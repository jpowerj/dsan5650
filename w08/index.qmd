...
| | Start | End | Topic |
|:- |:- |:- |:- |
| | 6:30pm | 7:00pm | [Final Project Pep Talk](#final-projects)
| **Lecture** | 7:00pm | 7:50pm | [Controlling For Things vs. Matching Things &rarr;](#propensity-score-weighting) |
...
## Working Example: Growth Mindset(!) {.smaller .title-09 .crunch-title .crunch-ul}
...
![](images/pgm_propensity_nocovar.svg){fig-align="center" width="400"}
...
# stud_mean_df <- tibble(x=c(0,1), y=c(ey0, ey1))
student_df |>
...
## How Exactly Do We *Adjust For* $\hat{\mathtt{e}}(\mathbf{X})$? {.smaller .title-12}
...
smoke_df |> mutate(enroll=factor(enroll)) |>
  ...
  # Predictions
  geom_point(
    aes(y=as.numeric(enroll))
  ) +
  # Values
  geom_point(
    aes(y=ipw, color=enroll)
  ) +
  theme_dsan(base_size=24)
```
...
```{r}
#| label: ipw-reg
lm(ncigs ~ enroll, data=smoke_df, weights=smoke_df$ipw)
```
