...
<thead>
<tr>
  <th>What Jeff calls it</th>
  <th>Citation (click for PDF link)</th>
</tr>
</thead>
<tbody>
<tr class='td-nb'>
  <td><span data-qmd='**"The WebPPL interactive textbook"**'></span></td>
  <td><span data-qmd="@goodman_probabilistic_2016"></span></td>
</tr>
<tr>
  <td colspan='2' class='tdc td-nb' style="padding-top: 0px;"><span data-qmd="[**5650 Edition Here**](https://jjacobs.me/probmods)"></span></td>
</tr>
<tr>
  <td><span data-qmd='**"Statistical Rethinking"**'></span></td>
  <td><span data-qmd='@mcelreath_statistical_2020'></span></td>
</tr>
<tr>
  <td><span data-qmd='**"The Multilevel book"**'></span></td>
  <td><span data-qmd='@gelman_data_2007'></span></td>
</tr>
<tr>
  <td><span data-qmd='**"The Koller PGM book"**'></span></td>
  <td><span data-qmd='@koller_probabilistic_2009'></span></td>
</tr>
<tr>
  <td><span data-qmd='**"The non-technical Pearl book"**'></span></td>
  <td><span data-qmd='@pearl_book_2018'></span></td>
</tr>
<tr>
  <td><span data-qmd='**"The technical Pearl book"** (containing, e.g., axioms and proofs)'></span></td>
  <td><span data-qmd='@pearl_causality_2000'></span></td>
</tr>
<tr>
  <td><span data-qmd='**"The French Revolution paper"**'></span></td>
  <td><span data-qmd='@barron_individuals_2018'></span></td>
</tr>
</tbody>
</table>
```
...
Contains info on Bayesian Workflow throughout, plus important chapters on **Multilevel Modeling** (Ch 5) and **Bayesian Decision Analysis** (Ch 9) -- the latter was referenced in the [writeup on Bayesian Workflow](./writeups/bayesian-workflow/)
...
<i class='bi bi-1-circle'></i> **@sperber_explaining_1996, *Explaining Culture: A Naturalistic Approach***
...
<i class='bi bi-1-circle'></i> **@titelbaum_fundamentals_2022, *Fundamentals of Bayesian Epistemology***

This book really only comes into play in Week 12, where we focus in very specifically on just two chapters (out of order relative to the book, but in order relative to the lecture topics!):

*Chapter 9: Dutch Book Arguments*, specifically pages 315 to 316, goes over the argument for using Bayes' Rule to update prior knowledge in light of new information (the gist being that, any *other* updating rule will lead to beliefs that render you susceptible to a series of bets that are "fair" to you under these beliefs, but which are actually guaranteed to lose money)

*Chapter 7: Decision Theory* provides a really great "walkthrough" of how one can **fuse** Bayesian probability with a (consequentialist) analysis of optimal decision-making. That is, once you have used Bayes' Rule to derive a posterior distribution over states of the world (optimally incorporating the information available to you, per the Dutch Book argument!), how can you optimally **act on** the posterior probabilities you've derived?

Intuitively, you might start by assigning **utility values** to actions and outcomes (how good or bad it would be for you to take action $a$ vs. action $b$ if the state of the world turns out to be $S_1$ vs. $S_2$), and then choosing the action that produces the highest **expected utility**. This approach works for decisions like whether to bring an umbrella on a trip, since your choice won't affect the probability of rain, but *fails* when the decision itself affects the outcome probabilities (like deciding whether or not to study for a test). To "fix" this issue, you need some *methodology* for **estimating the causal impacts** that your decisions will have on the probabilities of each outcome... in other words, the topics from Weeks 1 to 11 of this very course 😉
...
<i class='bi bi-1-circle'></i> **@hernan_causal_2025, *Causal Inference: What If?***

You can think of this as, it would be a perfect textbook for a "Part 2" of this course. This also means, however, that it's **perfect for handling "corner cases"** that you might run into while working on e.g. your Final Project! For instance, it has chapters explicitly walking through how you might handle datasets with obvious **measurement bias**, **selection bias**, and so on.
