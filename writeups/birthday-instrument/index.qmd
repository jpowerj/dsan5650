...
student_df |> group_by(dob) |> summarize(sum(can_drop_out))
```

```{r}
(elig_prop <- student_df |> summarize(elig_prop=mean(can_drop_out)) |> pull())
#raw_dropout_rate <- 0.046
raw_dropout_rate <- 0.086
(dropout_rate <- (1/elig_prop) * raw_dropout_rate)
```

* https://dcps.dc.gov/sites/default/files/dc/sites/dcps/publication/attachments/SY%2024-25%20Calendar_English_Updated_022625.pdf
